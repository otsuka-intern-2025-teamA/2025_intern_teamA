...
from openai import AzureOpenAI  # Новый импорт
...
class Briefing:
    ...
    def __init__(self) -> None:
        ...
        if not self.azure_endpoint or not self.azure_api_key:
            raise ValueError("AZURE_OPENAI_ENDPOINT or AZURE_OPENAI_API_KEY environment variable is not set")
        self.model = "gpt-5-mini"  # или другой доступный в Azure
    ...
    async def generate_category_briefing(
        ...
        briefing_start_time = time.perf_counter()
...
"""
        ...
        try:
            ...
            briefing_time = time.perf_counter() - briefing_start_time
            logger.info(f"✅ {category} briefing generated in {briefing_time:.2f} seconds")
            ...
            if websocket_manager := context.get('websocket_manager'):
                if job_id := context.get('job_id'):
                    await websocket_manager.send_status_update(
                        ...
                        message=f"Completed {category} briefing in {briefing_time:.2f}s",
                        result={
                            "step": "Briefing",
                            "category": category,
                            "generation_time": f"{briefing_time:.2f}s"
                        }
                    )
            ...
            briefing_time = time.perf_counter() - briefing_start_time
            logger.error(f"❌ Error generating {category} briefing after {briefing_time:.2f}s: {e}")
            return {'content': ''}
    ...
    async def create_briefings(self, state: ResearchState) -> ResearchState:
        ...
        if briefing_tasks:
            ...
            total_briefing_time = time.perf_counter() - briefing_start_time
            
            logger.info(f"🎯 Generated {successful_briefings}/{len(briefing_tasks)} briefings")
            logger.info(f"📊 Total content length: {total_length} characters")
            logger.info(f"⏱️  Total briefing generation time: {total_briefing_time:.2f} seconds")
