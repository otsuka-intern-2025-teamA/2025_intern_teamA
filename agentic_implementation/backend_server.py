...
if mongo_uri := os.getenv("MONGODB_URI"):
    try:
        ...
        logger.warning(f"⚠️ Failed to initialize MongoDB: {e}. Continuing without persistence.")
...
async def process_research(job_id: str, data: ResearchRequest):
    try:
        ...
        logger.info(f"🚀 Starting research for {data.company} at {datetime.now().strftime('%H:%M:%S')}")
        ...
        if report_content:
            ...
            logger.error(f"❌ Research failed for {data.company} after {execution_time:.2f} seconds")
        ...
        logger.error(f"💥 Research crashed for {data.company} after {execution_time:.2f} seconds: {str(e)}")
        await manager.send_status_update(
            job_id=job_id,
            status="failed",
