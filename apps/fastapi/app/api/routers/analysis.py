...
def analyze_company(
    ...
    if not item_id or not question:
        raise HTTPException(
            ...
            detail="item_id and question are required"
        )
...
def load_history(
    ...
    if not item_id or not company_name:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="item_id and company_name are required"
        )
    ...
    try:
        ...
        # 現在のロード状況を確認
        history_count = db.query(func.count(History.id)).filter(
