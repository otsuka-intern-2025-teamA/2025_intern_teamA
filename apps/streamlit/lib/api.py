...
class APIClient:
    ...
    def get_messages(
        self, item_id: str, skip: int = 0, limit: int = 50, search: str | None = None
    ) -> list[dict[str, Any]]:
        """案件内のメッセージ一覧を取得"""
    ...
    def analyze_company(
        self, item_id: str, question: str, company_name: str | None = None, top_k: int = 5
    ) -> dict[str, Any]:
        """企業分析を実行"""
        data = {"item_id": item_id, "question": question, "top_k": top_k}
        if company_name:
    ...
    def load_history(self, item_id: str, company_name: str) -> dict[str, Any]:
        """取引履歴をロード"""
        data = {"item_id": item_id, "company_name": company_name}
