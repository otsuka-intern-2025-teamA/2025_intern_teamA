...
class Editor:
    ...
    def __init__(self) -> None:
        ...
        if not self.azure_api_key or not self.azure_endpoint:
            raise ValueError("AZURE_OPENAI_API_KEY or AZURE_OPENAI_ENDPOINT environment variable is not set")
    ...
    async def compile_briefings(self, state: ResearchState) -> ResearchState:
        ...
        logger.info(f"⏱️  Briefing compilation completed in {compile_time:.2f} seconds")
    ...
    async def edit_report(self, state: ResearchState, briefings: Dict[str, str], context: Dict[str, Any]) -> str:
        ...
        try:
            ...
            logger.info(f"🚀 Starting final report editing for {company}")
            ...
            edit_time = time.perf_counter() - edit_start_time
            logger.info(f"✅ Final report compiled in {edit_time:.2f} seconds with {len(final_report)} characters")
            ...
            if websocket_manager := state.get('websocket_manager'):
                if job_id := state.get('job_id'):
                    await websocket_manager.send_status_update(
                        ...
                        message=f"Research report completed in {edit_time:.2f}s",
                        result={
                            "step": "Editor",
                            "report": final_report,
                            "company": company,
            ...
            edit_time = time.perf_counter() - edit_start_time
            logger.error(f"❌ Error in edit_report after {edit_time:.2f}s: {e}")
            return ""
...
Return the cleaned report in flawless markdown format. No explanations or commentary."""
        ...
        try:
            ...
            # Send completion status to the UI
            if websocket_manager := state.get('websocket_manager'):
                if job_id := state.get('job_id'):
                    await websocket_manager.send_status_update(
                        job_id=job_id,
                        status="report_chunk",
                        message="Report formatting completed",
                        result={
                            "chunk": formatted_report,
                            "step": "Editor"
                        }
                    )
