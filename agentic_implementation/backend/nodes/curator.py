...
class Curator:
    ...
    async def evaluate_documents(self, state: ResearchState, docs: list, context: Dict[str, str]) -> list:
        ...
        logger.info(f"✅ Document evaluation completed in {eval_time:.2f}s: {len(evaluated_docs)} documents kept")
        
        return evaluated_docs

    async def curate_data(self, state: ResearchState) -> ResearchState:
        """Curate all collected data based on Tavily scores."""
        curation_start_time = time.perf_counter()
        ...
        curation_time = time.perf_counter() - curation_start_time
        total_initial = sum(counts["initial"] for counts in doc_counts.values())
        total_kept = sum(counts["kept"] for counts in doc_counts.values())
        
        logger.info(f"🎯 Curation completed in {curation_time:.2f}s: {total_kept}/{total_initial} documents kept")
        logger.info(f"📊 Document retention rate: {(total_kept/total_initial*100):.1f}%" if total_initial > 0 else "📊 No documents to curate")
