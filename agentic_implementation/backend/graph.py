...
class Graph:
    def __init__(self, company=None, url=None, hq_location=None, industry=None,
        ...
        self.job_id = job_id
        self.start_time = time.perf_counter()  # Засекаем общее время начала
    ...
    async def run(self, thread: Dict[str, Any]) -> AsyncIterator[Dict[str, Any]]:
        ...
        logger.info(f"🚀 Starting research workflow for {self.input_state.get('company', 'Unknown Company')}")
        logger.info(f"⏰ Workflow started at: {time.strftime('%H:%M:%S')}")
        ...
        logger.info(f"✅ Research workflow completed in {total_time:.2f} seconds")
        logger.info(f"⏰ Workflow completed at: {time.strftime('%H:%M:%S')}")
